...
class GoogleTrendsScraper:
	...
	def build_state(self, begin, end, symbols):
		# Data is absent
		f = datetime.strptime(os.environ.get("TRENDS_MIN_DATE"), "%Y-%m-%d %H:%M:%S")
	...
	def save(self, state, dir = 'output'):
		...
		for c, chunk in state.items():
			...
			merged = pd.concat([pd.read_csv(f, index_col=None, header=0, sep=',', encoding='utf-8') for f in files], axis=0, ignore_index=True)
			# Sort by date
